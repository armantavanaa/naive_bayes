...
def vocab(neg: Sequence[Sequence], pos: Sequence[Sequence]) -> dict:
    ...
    comb = sorted(set(new_neg).union(set(new_pos)))
    V['__unknown__'] = 0
    i = 1
    for word in comb:
        V[word] = i
...
class NaiveBayes621:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        xneg = X[np.where(y == 0)]
        xpos = X[np.where(y == 1)]
        w_neg_count = xneg.sum()
        w_pos_count = xpos.sum()
        self.p_c_neg = w_neg_count / (w_pos_count + w_neg_count)
        self.p_c_pos = w_pos_count / (w_pos_count + w_neg_count)
        self.p_w_neg = (xneg.sum(axis=0) + 1) / (w_neg_count + len(X[0]))
        self.p_w_pos = (xpos.sum(axis=0) + 1) / (w_pos_count + len(X[0]))
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        res = []
        for x in X:
            n = np.log(self.p_c_neg) + np.sum(x @ np.log(self.p_w_neg))
            p = np.log(self.p_c_pos) + np.sum(x @ np.log(self.p_w_pos))
            if n > p:
                res.append(0)
            else:
                res.append(1)
